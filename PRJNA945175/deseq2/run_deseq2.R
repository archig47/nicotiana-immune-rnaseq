...
counts_file <- "/users/fyp/fyp5/project/PRJNA945175/featurecounts/counts_v12_multimapper.txt"
...
cat("Loading counts...\n")
raw    <- read.table(counts_file, header=TRUE, sep="\t", comment.char="#", check.names=FALSE)
counts <- raw[, 7:ncol(raw)]
rownames(counts) <- gsub("-mRNA$", "", raw$Geneid)
colnames(counts) <- gsub(".*/aligned/(SRR[0-9]+)/.*", "\\1", colnames(counts))
counts <- round(counts)
...
coldata <- data.frame(
  sample    = c("SRR23875051","SRR23875052","SRR23875053","SRR23875054",
                ...
                "SRR23875079","SRR23875080","SRR23875082",
...
cat("Samples in analysis:", ncol(counts), "\n")

cat("Running DESeq2 with design ~ timepoint + condition...\n")
